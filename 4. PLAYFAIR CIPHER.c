...
main()
{
 int i,j,k,numstr[100],numcipher[100],numcipher2[100],numkey[100],lenkey,templen,tempkey[100],flag=-1,size,cipherkey[5][5],lennumstr,row1,row2,col1,col2;
 ...
 for(i=0;i<5;i++)
 {
  ...
  printf("\n");
   ...
   {
      if(numstr[i]==numstr[i+1]) //if repeated element add 'x' to the repeated element
      {
       insertelementat(i+1,numstr,lennumstr);
       lennumstr++;
      }
   }
   if(lennumstr%2!=0) //if length of string is not even add 'x' to the last to make it even
   {
 ...
 //Encryption Of String
   for(k=0;k<lennumstr;k+=2)
   {
    for(i=0;i<5;i++)
    {
     for(j=0;j<5;j++)
     {
      if(numcipher[k]==cipherkey[i][j])
      {
         row1=i;
         col1=j;
      }
      if(numcipher[k+1]==cipherkey[i][j])
      {
         row2=i;
         col2=j;
      }
    ...
    if(row1==row2)
    {
     ...
     numcipher2[k]=cipherkey[row1][col1];
     numcipher2[k+1]=cipherkey[row2][col2];
    }
    if(col1==col2)
    {
     row1=(row1-1)%5;
    ...
    if(row1!=row2&&col1!=col2)
    {
     numcipher2[k]=cipherkey[row1][col2];
     numcipher2[k+1]=cipherkey[row2][col1];
    }
   }
   printf("\nDecrypted Text is\n");
   for(i=0;i<lennumstr;i++)
   {
    if((numcipher2[i]+'A')!='X')//Should remove extra 'X' which were created during Encryption
      printf("%c",numcipher2[i]+'A');
   }
   printf("\n");
}
int removerepeated(int size,int a[])
